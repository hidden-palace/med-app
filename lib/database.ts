...
export async function getAllValidationHistory(limit: number = 50) {
  // Fetch via server API to centralize RLS/admin checks
  ...
  try {
    ...
    const res = await fetch(`/api/admin/validations?limit=${safeLimit}`, {
      method: 'GET',
    ...
    if (!res.ok) {
      ...
      throw new Error(`Failed to load validations (${res.status}). ${body}`)
    }

    const data = await res.json()
    return data as (ValidationHistory & { profiles: Pick<Profile, 'full_name' | 'email'> | null })[]
  } catch (error) {
    console.error('Error in getAllValidationHistory:', error)
    throw error
  }
}
