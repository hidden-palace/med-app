...
function getSessionFingerprint(session: Session | null | undefined): string | null {
...
export default function Home() {
  ...
  const lastHandledSignInFingerprintRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      try {
        ...
        setUser(currentUser);
        lastHandledSignInFingerprintRef.current = getSessionFingerprint(session);
        ...
        if (currentUser) {
          ...
          adminStatusRef.current = false;
          lastAdminCheckUserIdRef.current = null;
          lastHandledSignInFingerprintRef.current = null;
        }
      } catch (error) {
    ...
    const {
      ...
      const isNewSignInSession =
        event === 'SIGNED_IN' &&
        !!sessionFingerprint &&
        lastHandledSignInFingerprintRef.current !== sessionFingerprint;

      if (event === 'TOKEN_REFRESHED' && sessionFingerprint) {
        lastHandledSignInFingerprintRef.current = sessionFingerprint;
      }

      if (currentUser && adminCheckEvents.includes(event)) {
        if (isNewSignInSession) {
          lastHandledSignInFingerprintRef.current = sessionFingerprint;
          updateProfile(
            currentUser.id,
        ...
        const shouldForceCheck = event === 'USER_UPDATED' || isNewSignInSession;
