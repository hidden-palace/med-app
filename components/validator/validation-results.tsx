...
const PRIORITY_ORDER: NormalizedRecommendation["priority"][] = [
  "high",
  "medium",
  "low",
];
export function ValidationResults({ result }: ValidationResultsProps) {
  const normalized = useMemo(
    () => normalizeValidationDetails(result),
    [result],
  );
      ...
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-2">
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            <Badge
              variant="outline"
              className={statusBadgeClasses(validationStatus)}
            >
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  ...
                  <OverviewStat
                    label="State / Region"
                    value={`${result.state} / ${result.region}`}
                    icon={MapPin}
                  />
                  <OverviewStat
                    label="Validated"
                ...
                <Alert
                  ...
                  className={cn(
                    "border-l-4",
                    statusAccentBorder(validationStatus),
                  )}
                >
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    <p className="font-semibold">
                      {normalized.overallSummary.summaryText ||
            ...
            {(nextSteps.length > 0 || recommendations.length > 0) && (
              <Card>
                <CardHeader className="pb-4">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Lightbulb className="h-4 w-4 text-primary" /> Prioritized
                    Next Steps
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
          ...
          <TabsContent value="clinical" className="space-y-4">
            <Card>
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Stethoscope className="h-4 w-4 text-primary" /> Clinical
                  Narrative
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Accordion type="multiple" className="space-y-2">
                  {renderClinicalSection(
                    "Chief Complaint",
                    normalized.sections.chiefComplaint,
                  )}
                  {renderClinicalSection(
                  ...
                  {renderClinicalSection(
                    ...
                    normalized.sections.interventions,
                  )}
                  {renderClinicalSection(
                    "Treatment Plan",
                    normalized.sections.plan,
                  )}
                  {renderClinicalSection(
                    "Medical Necessity",
                    normalized.sections.medicalNecessity,
                  )}
                  {renderClinicalSection(
                    "Comorbidities",
                    normalized.sections.comorbidities,
                  )}
                  {renderClinicalSection(
                    "Consent",
                    normalized.sections.consent,
                  )}
                  {renderClinicalSection(
                    "Supporting Documentation",
                    normalized.sections.documentation,
                  )}
                </Accordion>
              </CardContent>
            </Card>
          ...
          <TabsContent value="wound" className="space-y-4">
            <Card>
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Thermometer className="h-4 w-4 text-primary" /> Wound
                  Assessment
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {normalized.woundAssessment ? (
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    {renderAssessmentRow(
                      "Location",
                      normalized.woundAssessment.location,
                      MapPin,
                    )}
                    {renderAssessmentRow(
                      "Edges",
                      normalized.woundAssessment.edges,
                      Layers,
                    )}
                    {renderAssessmentRow(
                      "Base",
                      normalized.woundAssessment.base,
                      ShieldCheck,
                    )}
                    {renderAssessmentRow(
                      "Exudate",
                      normalized.woundAssessment.exudate,
                      Droplets,
                    )}
                    {renderAssessmentRow(
                      "Infection Signs",
                      normalized.woundAssessment.infectionSigns,
                      AlertCircle,
                    )}
                    {renderAssessmentRow(
                      "Surrounding Skin",
                      normalized.woundAssessment.surroundingSkin,
                      Circle,
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">
                    No wound assessment details were provided.
                  </p>
                )}
                ...
                {normalized.woundAssessment?.size && (
                  <div className="rounded-lg border bg-muted/50 p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {Object.entries(normalized.woundAssessment.size).map(
                        ([label, value]) => (
                          <div key={label}>
                            <div className="text-xs uppercase text-muted-foreground">
                              {formatLabel(label)}
                ...
                {normalized.sections.photosMeasurements && (
                  <div className="rounded-lg border bg-muted/30 p-4">
                    <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
                      <ImageIcon className="h-4 w-4" /> Photos & Measurement
                      Notes
                    </h4>
                  ...
                  <CardHeader className="pb-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={lcdStatusBadgeClasses(lcd.status)}>
                          {lcdStatusLabel(lcd.status)}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
...
function normalizeValidationDetails(
  result: ValidationHistory,
): NormalizedValidationDetails {
  const rawDetails = parseDetails(result.result_details);
  const overallSummary =
    rawDetails?.overallSummary ?? rawDetails?.summary ?? {};
  ...
  const aggregatedRecommendations = aggregateRecommendations(
    rawDetails,
    lcdChecks,
  );
  ...
  return {
    ...
    overallSummary: {
      status: (
        overallSummary.complianceStatus ??
...
function buildMeta(
  ...
  return {
    ...
    generatedAt: obj.generatedAt
      ? new Date(obj.generatedAt).toLocaleString()
      : undefined,
...
function buildWoundAssessment(
  raw: any,
): NormalizedValidationDetails["woundAssessment"] {
...
function buildLCDChecks(rawDetails: any, fallback: any): NormalizedLCDCheck[] {
  const lcdSource =
    rawDetails?.lcdChecks ??
    rawDetails?.lcd_results ??
    rawDetails?.lcdCompliance ??
    fallback;
...
function normalizeLCDCheck(entry: any, index: number): NormalizedLCDCheck {
  ...
  const status = normalizeLCDStatus(
    obj.status ?? obj.outcome ?? obj.compliance ?? "unknown",
  );
  ...
  const reasons = toStringArray(
    obj.reasons ??
      obj.reason ??
      obj.details ??
      obj.missing_elements ??
      obj.assessment,
  );
  const evidence = toStringArray(
    obj.evidence ??
      obj.supportingEvidence ??
      obj.documentation ??
      obj.documents,
  );
  const recommendations = normalizeRecommendationCollection(
    obj.recommendations ?? obj.suggestions ?? obj.actions,
    {
      priority: "medium",
      source: title,
    },
...
function normalizeLCDStatus(value: unknown): NormalizedLCDCheck["status"] {
  ...
  if (
    ["partial", "partially met", "warning", "needs improvement"].includes(input)
  ) {
    return "partial";
  }
...
function aggregateRecommendations(
  ...
  const push = (items: NormalizedRecommendation[]) => {
    for (const item of items) {
      ...
      if (
        !aggregate.some(
          (existing) =>
            `${existing.priority}-${existing.text.toLowerCase()}` === key,
        )
      ) {
        aggregate.push(item);
      }
  ...
  push(
    normalizeRecommendationCollection(rawDetails?.recommendations, {
      priority: "medium",
      source: "AI Analysis",
    }),
  );
  push(
    normalizeRecommendationCollection(
...
function normalizePriority(
  value: unknown,
): NormalizedRecommendation["priority"] {
  const input = (value ?? "").toString().toLowerCase();
...
function resolveValidationStatus(
  ...
  if (
    ["pass", "passed", "compliant"].includes(normalizedStatus) ||
    score >= 90
  ) {
    return "passed";
  }
  ...
  if (
    ["warning", "partial", "partially compliant"].includes(normalizedStatus) ||
    score >= 70
  ) {
    return "warning";
  }
    ...
    <Card className="border-primary/20 bg-primary/5">
      ...
      <CardContent className="space-y-2 text-sm text-muted-foreground">
        {recommendations.map((rec) => (
          <div
            key={rec.id}
            className="flex flex-col gap-1 rounded-md border bg-background/80 p-3"
          >
            <span className="font-medium text-foreground">{rec.text}</span>
            <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              <Badge
                variant="outline"
                className={priorityBadgeClasses(rec.priority)}
              >
...
function priorityBadgeClasses(
  priority: NormalizedRecommendation["priority"],
): string {
  switch (priority) {
    case "high":
      return "bg-red-50 text-red-700 border-red-200";
...
function buildReportText(
  ...
  if (normalized.lcdChecks.length === 0) {
    ...
    normalized.lcdChecks.forEach((lcd, idx) => {
      ...
      lines.push(
        `   Status: ${lcdStatusLabel(lcd.status)}${lcd.score ? ` (${lcd.score}%)` : ""}`,
      );
      if (lcd.summary) {
